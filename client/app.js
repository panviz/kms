...
class App {
  ...
  async showChildren (keyS, dbclick) {
    ...
    // TODO multiple
    if (dbclick) {
      const graph = await App._request('getGraph', rootKey, 1)
      this._filter(graph)
  ...
  async linkItems (source, targets) {
    await App._request('associate', source, targets)
    await this._reloadGraph(this.serviceItem.visibleItem)
    this._updateGraphView({ tags: [], notes: [] })
  }

  async unlinkItems (source, targets) {
    await App._request('setDisassociate', source, targets)
    await this._reloadGraph(this.serviceItem.visibleItem)
    this._updateGraphView({ tags: [], notes: [] })
  }
   ...
   */
  async _loadRepo () {
    const graph = await App._request('getGraph', this.rootKey, 1)
    ...
    // проеверить если связан с типом но скрыт???
    await this._reloadGraph([this.serviceItem.visibleItem,
